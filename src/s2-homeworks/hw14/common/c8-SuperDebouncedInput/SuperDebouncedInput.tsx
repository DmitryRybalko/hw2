...
const SuperDebouncedInput: React.FC<SuperDebouncedInputPropsType> = ({
  ...
  const onChangeTextCallback = (value: string) => {
    ...
    if (onDebouncedChange) {
      if (timerId) {
        clearTimeout(timerId); // остановить предыдущий таймер, если он был установлен
      }
      const newTimerId = setTimeout(() => {
        onDebouncedChange(value); // вызов функции onDebouncedChange после задержки
      }, 1500); // установить нов таймер на 1500ms
      setTimerId(+newTimerId); // сохранить идентификатор нового таймера в состоянии
    }
  };
